    ...
    trap(height) {
        ...
        while (i < height.length - 1) {
            ...
            while (j < height.length) {
                ...
                if (smallerLeft && smallerRight) {
                    if (height[j] > maxima) {
                        maxima = height[j];
                        maximaIdx = j;
                        console.log(`Maxima at ${j} of ${height[j]}`);
                    }
                }
            ...
            for (let k = i + 1; k < maximaIdx; k++) {
                ...
                console.log(`Filling at ${k}, total: ${filled}`);
            }

            let curArea = waterHeight * (maximaIdx - i - 1);
            curArea -= filled;
            area += Math.max(curArea, 0);
            console.log(`Cur area: ${curArea}, Total: ${area}`);
            i = maximaIdx;
        }
